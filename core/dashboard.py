...
from textual.widgets import Header, Footer, Static, DataTable, Tree, Label
    ...
    def load_resources(self, resource_type: str):
        ...
        for acc in accounts:
            ...
            try:
                ...
                # Mock 数据用于展示 TUI 效果
                table.add_row(f"i-{resource_type}-001", f"{acc.name}-{resource_type}-1", "cn-hangzhou", "Running")
                table.add_row(f"i-{resource_type}-002", f"{acc.name}-{resource_type}-2", "cn-beijing", "Stopped")
                count += 2
            except Exception:
                pass
