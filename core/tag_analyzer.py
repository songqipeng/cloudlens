...
class TagAnalyzer:
    ...
    @staticmethod
    def check_tag_naming_convention(resources: List[UnifiedResource], conventions: Dict = None) -> List[Dict]:
        """
        检查标签命名规范
        ...
        for r in resources:
            ...
            if conventions.get("required_keys"):
                missing_keys = set(conventions["required_keys"]) - set(r.tags.keys() if r.tags else [])
                if missing_keys:
    ...
    def suggest_tag_optimization(resources: List[UnifiedResource]) -> List[str]:
        ...
        if coverage['coverage_rate'] < 50:
            suggestions.append(f"⚠️ 标签覆盖率仅{coverage['coverage_rate']}%，建议为所有资源添加标签以便管理")
        
        if coverage['untagged'] > 0:
            suggestions.append(f"发现{coverage['untagged']}个无标签资源，建议至少添加 env, project, owner 标签")
        
        tag_keys_info = TagAnalyzer.analyze_tag_keys(resources)
        if tag_keys_info['unique_keys'] > 20:
            suggestions.append(f"⚠️ 发现{tag_keys_info['unique_keys']}个不同的标签键，建议统一标签命名规范")
