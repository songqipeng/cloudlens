...
class TopologyGenerator:
    ...
    @staticmethod
    def generate_mermaid(instances: List[UnifiedResource], vpcs: List[Dict], rds_instances: List[UnifiedResource] = None) -> str:
        """
        生成Mermaid格式的网络拓扑图
            ...
            for az in sorted(azs_in_vpc):
                ...
                if key in vpc_az_instances:
                    for inst in vpc_az_instances[key]:
                        ip = inst.public_ips[0] if inst.public_ips else (inst.private_ips[0] if inst.private_ips else "N/A")
                        status_emoji = "🟢" if inst.status.value == "Running" else "🔴"
                        lines.append(f"      {inst.id}[\"{status_emoji} {inst.name}<br/>{ip}<br/>{inst.spec}\"]")
    ...
    @staticmethod
    def generate_markdown_report(account_name: str, instances: List[UnifiedResource], 
                                   vpcs: List[Dict], rds_instances: List[UnifiedResource] = None,
                                   redis_instances: List[UnifiedResource] = None,
                                   eips: List[Dict] = None) -> str:
        """
        生成完整的Markdown报告
        ...
        if instances:
            ...
            for inst in instances:
                ...
                lines.append(f"| {inst.id} | {inst.name} | {inst.status.value} | {pub_ip} | {priv_ip} | {inst.spec} | {inst.vpc_id or '-'} |")
            lines.append("")
        ...
        if rds_instances:
            ...
            lines.append("|---|---|---|---|---|")
            for rds in rds_instances:
                lines.append(f"| {rds.id} | {rds.name} | {rds.status.value} | {rds.spec} | {rds.vpc_id or '-'} |")
            lines.append("")
