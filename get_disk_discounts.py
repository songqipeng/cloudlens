...
def get_disk_renewal_price(client, region_id, disk_id, disk_category, disk_size, instance_id=None, disk_role='', all_disks_for_instance=None):
    """获取云盘的续费价格
                ...
                if total_original_price > 0 or total_trade_price > 0:
                    ...
                    if disk_type == 'dataDisk' and all_disks_for_instance:
                        ...
                        total_data_disk_size = sum(
                            ...
                            if d.get('实例ID') == instance_id and d.get('磁盘角色', '').lower() != 'system'
                        )
...
def main(tenant_name=None):
    ...
    disk_files = [f for f in os.listdir(reports_dir) if f.startswith(f'{tenant_name}_ecs_disks_') and f.endswith('.xlsx')]
    ...
    disk_items = [
        (row, access_key_id, access_key_secret, prepaid_disks_dict)
        for row in prepaid_disks_dict
    ]
    ...
    for _, row in prepaid_disks.iterrows():
        disk_id = row['磁盘ID']
        price_info = price_dict.get(disk_id, {
            'original_price': 0,
            'trade_price': 0,
            'estimated': False,
            'error': '未查询到'
    ...
    for min_d, max_d, label in discount_ranges:
        if len(valid_discounts) > 0:
            count = len(valid_discounts[
                (valid_discounts['折扣率(%)'] >= min_d) & 
                (valid_discounts['折扣率(%)'] < max_d)
            ])
            if count > 0:
