...
class NetworkAnalyzer:
                    ...
                    if 'VSwitches' in data and 'VSwitch' in data['VSwitches']:
                        ...
                        for vswitch in vswitch_list:
                            detail['VSwitches'].append({
                                ...
                                'AvailableIpAddressCount': vswitch.get('AvailableIpAddressCount', 0),
    ...
    def get_vpc_peerings(self, region: str) -> List[Dict]:
        ...
        try:
            ...
            while True:
                ...
                if 'VpcPeerConnections' in data and 'VpcPeerConnection' in data['VpcPeerConnections']:
    ...
    def get_express_connect(self, region: str) -> List[Dict]:
        ...
        try:
            ...
            while True:
                ...
                if 'PhysicalConnectionSet' in data and 'PhysicalConnectionType' in data['PhysicalConnectionSet']:
                    ...
                    for conn in conn_list:
                        connections.append({
                            ...
                            'RedundantPhysicalConnectionId': conn.get('RedundantPhysicalConnectionId', ''),
                ...
                while True:
                    ...
                    if 'Listeners' in data and 'Listener' in data['Listeners']:
                        ...
                        for listener in listener_list:
                            # 获取健康检查配置
                            health_check = self.get_slb_health_check(region, instance_id, 
                                                                    listener.get('ListenerPort', 0), lb_type)
    ...
    def get_slb_health_check(self, region: str, instance_id: str, 
                           listener_port_or_id, lb_type: str = 'clb') -> Dict:
        """获取SLB健康检查配置"""
        try:
            client = AcsClient(self.access_key_id, self.access_key_secret, region)
                ...
                if 'BackendServers' in data and 'BackendServer' in data['BackendServers']:
                    ...
                    if backend_list:
                        return {
                            ...
                            'HealthCheckConnectPort': backend_list[0].get('HealthCheckConnectPort', 0),
    ...
    def get_slb_server_groups(self, region: str, instance_id: str, lb_type: str = 'clb') -> List[Dict]:
        """获取SLB后端服务器组详情"""
        server_groups = []
        try:
            client = AcsClient(self.access_key_id, self.access_key_secret, region)
    ...
    def get_server_group_servers(self, region: str, server_group_id: str, lb_type: str) -> List[Dict]:
        """获取服务器组中的服务器列表"""
        servers = []
        try:
            client = AcsClient(self.access_key_id, self.access_key_secret, region)
    ...
    def get_cdn_domain_detail(self, domain_name: str) -> Dict:
        ...
        try:
            ...
            try:
                ...
                request.add_query_param('DomainName', domain_name)
                request.add_query_param('FunctionNames', 'set_req_host_header,filetype_based_ttl_setting,path_based_ttl_setting')
...
'''
            for vpc in results['VPCs']:
                status_class = 'badge-success' if vpc.get('Status') == 'Available' else 'badge-warning'
                html_content += f'''
                    <tr>
...
'''
            for peering in results['VpcPeerings']:
                status_class = 'badge-success' if peering.get('Status') == 'Active' else 'badge-warning'
                html_content += f'''
                    <tr>
...
'''
            for vpn in results['VpnConnections']:
                status_class = 'badge-success' if vpn.get('Status') == 'ike_sa_not_established' else 'badge-warning'
                html_content += f'''
                    <tr>
...
'''
            for conn in results['ExpressConnects']:
                status_class = 'badge-success' if conn.get('Status') == 'Enabled' else 'badge-warning'
                html_content += f'''
                    <tr>
...
'''
            for slb in results['SLBs']:
                status_class = 'badge-success' if slb.get('InstanceStatus') == 'active' else 'badge-warning'
                html_content += f'''
                    <tr>
...
'''
            for cdn in results['CDNs']:
                status_class = 'badge-success' if cdn.get('DomainStatus') == 'online' else 'badge-warning'
                html_content += f'''
                    <tr>
    ...
    def generate_network_report(self, results: Dict):
        ...
        try:
            ...
            if results['VPCs']:
                vpc_data = []
                for vpc in results['VPCs']:
                    vswitches_str = '\n'.join([f"{vs.get('VSwitchName', '')}({vs.get('CidrBlock', '')})" 
                                             for vs in vpc.get('VSwitches', [])])
                    route_tables_str = '\n'.join([f"{rt.get('RouteTableName', '')}" 
                                                 for rt in vpc.get('RouteTables', [])])
                    acls_str = '\n'.join([f"{acl.get('NetworkAclName', '')}" 
                                        for acl in vpc.get('NetworkAcls', [])])
