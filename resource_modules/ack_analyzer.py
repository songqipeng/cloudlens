...
@AnalyzerRegistry.register("ack", "ACK容器服务", "☸️")
class ACKAnalyzer(BaseResourceAnalyzer):
    """ACK资源分析器"""
    ...
    def get_instances(self, region: str):
        """获取指定区域的ACK集群 (BaseResourceAnalyzer接口)"""
        return self.get_ack_clusters(region)
    ...
    def get_metrics(self, region: str, instance_id: str, days: int = 14):
        """获取ACK集群的监控数据 (BaseResourceAnalyzer接口)"""
        return self.get_ack_metrics(region, instance_id)
    ...
    def is_idle(self, instance, metrics, thresholds=None):
        """判断ACK集群是否闲置 (BaseResourceAnalyzer接口)"""
        is_idle = self.is_ack_idle(metrics)
        conditions = []
        if is_idle:
    ...
    def get_optimization_suggestions(self, instance, metrics):
        ...
        return self.get_optimization_suggestion(metrics, instance.get("ClusterType", ""))
    ...
    def analyze(self, regions=None, days=14):
        ...
        # ACKAnalyzer的analyze_ack_resources逻辑比较复杂
        return self.analyze_ack_resources()

    def generate_report(self, idle_instances):
        """生成报告 (BaseResourceAnalyzer接口)"""
        self.generate_ack_report(idle_instances)
    ...
    def analyze_ack_resources(self):
        ...
        results = process_concurrently(
            all_regions, get_region_clusters, max_workers=10, description="ACK集群采集"
        )
        ...
        monitoring_results = process_concurrently(
            all_clusters, process_single_cluster, max_workers=10, description="ACK监控数据采集"
        )
        ...
        for cluster in all_clusters:
            ...
            if self.is_ack_idle(metrics):
                ...
                optimization = self.get_optimization_suggestion(
                    metrics, cluster.get("ClusterType", "")
                )
                monthly_cost = self.get_monthly_cost(
                    cluster_id, cluster.get("NodeCount", 0), cluster["RegionId"]
                )
