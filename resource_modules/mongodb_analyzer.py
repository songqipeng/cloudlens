...
class MongoDBAnalyzer:
    def __init__(self):
        ...
        self.regions = [
            ...
            'ap-northeast-1', 'ap-south-1', 'us-east-1', 'us-west-1',
    ...
    def save_to_database(self, instances, metrics_data):
        ...
        for instance in instances:
            ...
            instance_name = instance.get('InstanceName') or instance.get('DBInstanceDescription', 'unknown')
            instance_class = instance.get('InstanceClass') or instance.get('DBInstanceClass', 'unknown')
            engine_version = instance.get('EngineVersion') or instance.get('EngineVersion', 'unknown')
            ...
            instance_type = instance.get('InstanceType') or instance.get('DBInstanceType', 'unknown')
            storage_engine = instance.get('StorageEngine') or instance.get('StorageEngine', 'unknown')
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                instance_id, instance_name, instance_class,
                engine_version, region, status,
                instance_type, storage_engine, 
        ...
        for instance_id, metrics in metrics_data.items():
            for metric_name, metric_value in metrics.items():
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?)
                ''', (instance_id, metric_name, metric_value, datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
