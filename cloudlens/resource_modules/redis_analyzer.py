...
@AnalyzerRegistry.register("redis", "Redis数据库", "🔴")
class RedisAnalyzer(BaseResourceAnalyzer):
    """Redis资源分析器"""
    ...
    def get_instances(self, region: str):
        """获取指定区域的Redis实例 (BaseResourceAnalyzer接口)"""
        return self.get_redis_instances(region)
    ...
    def get_metrics(self, region: str, instance_id: str, days: int = 14):
        """获取Redis实例的监控数据 (BaseResourceAnalyzer接口)"""
        return self.get_redis_metrics(region, instance_id)
    ...
    def is_idle(self, instance, metrics, thresholds=None):
        """判断Redis实例是否闲置 (BaseResourceAnalyzer接口)"""
        is_idle = self.is_redis_idle(metrics)
        conditions = []
        if is_idle:
    ...
    def analyze(self, regions=None, days=14):
        ...
        return self.analyze_redis_resources()

    def generate_report(self, idle_instances):
        """生成报告 (BaseResourceAnalyzer接口)"""
        self.generate_redis_report(idle_instances)
    ...
    def analyze_redis_resources(self):
        ...
        region_results = process_concurrently(
            regions, get_region_instances, max_workers=10, description="获取Redis实例"
        )
        ...
        def process_single_instance(instance_item):
            ...
            try:
                ...
                ErrorHandler.handle_instance_error(
                    e, instance_id, region, "Redis", continue_on_error=True
                )
                return {
                    "success": False,
        ...
        for instance in all_instances:
            ...
            if self.is_redis_idle(metrics):
                ...
                monthly_cost = self.get_monthly_cost(
                    instance_id, instance["InstanceClass"], instance["Region"]
                )
    ...
    def generate_redis_report(self, idle_instances):
        ...
        self.logger.info(
            f"Redis闲置实例统计: 总数量={len(idle_instances)}个, 总月成本={total_cost:,.2f}元, 预计年节省={total_cost * 12:,.2f}元"
        )
