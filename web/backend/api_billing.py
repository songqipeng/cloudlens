...
def get_billing_data_status(account: Optional[str] = None):
    ...
    try:
        ...
        if not result or result[0]['total_records'] == 0:
            return {
                ...
                "data": {
                    ...
                    "missing_months": [],
                    "completion_rate": 0,
                    "message": "暂无账单数据，正在从云端同步..."
                }
            }

        stats = result[0]
        total_months = int(stats.get('total_months', 0))
        ...
        completion_rate = round((total_months / len(expected_months) * 100) if expected_months else 100, 1)
        ...
        return {
            ...
            "data": {
                ...
                "message": "数据完整" if is_complete else f"数据同步中... ({completion_rate}% 完成，缺少 {len(missing_months)} 个月)"
            }
        }

    except Exception as e:
        logger.error(f"获取账单数据状态失败: {str(e)}", exc_info=True)
        return {
            "success": False,
            "error": str(e),
            "message": "获取账单数据状态失败"
