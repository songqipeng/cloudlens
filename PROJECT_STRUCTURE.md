# CloudLens 项目结构

本文档详细说明 CloudLens 项目的目录结构和各模块的职责。

## 📁 目录结构

```
aliyunidle/
├── cli/                          # CLI命令行工具
│   ├── main.py                  # CLI主入口
│   ├── utils.py                 # CLI工具函数
│   └── commands/                # 命令模块
│       ├── analyze_cmd.py       # 分析命令
│       ├── bill_cmd.py          # 账单命令
│       ├── cache_cmd.py         # 缓存管理命令
│       ├── config_cmd.py        # 配置管理命令
│       ├── query_cmd.py         # 资源查询命令
│       └── remediate_cmd.py     # 修复命令
│
├── core/                         # 核心业务逻辑
│   ├── database.py              # 数据库抽象层（MySQL/SQLite）
│   ├── config.py                # 配置管理
│   ├── provider.py              # 云厂商抽象基类
│   ├── cache.py                 # 缓存管理器
│   ├── bill_storage.py          # 账单存储管理
│   ├── dashboard_manager.py     # 仪表盘管理
│   ├── budget_manager.py        # 预算管理
│   ├── alert_manager.py         # 告警管理
│   ├── virtual_tags.py          # 虚拟标签管理
│   ├── cost_allocation.py       # 成本分配
│   ├── idle_detector.py         # 闲置资源检测
│   ├── cost_analyzer.py         # 成本分析
│   ├── cost_trend_analyzer.py   # 成本趋势分析
│   ├── discount_analyzer_advanced.py  # 高级折扣分析
│   ├── security_compliance.py   # 安全合规检查
│   ├── cis_compliance.py        # CIS基准检查
│   ├── tag_analyzer.py          # 标签分析
│   ├── report_generator.py      # 报告生成
│   ├── filter_engine.py         # 高级筛选引擎
│   ├── rules_manager.py         # 规则管理
│   ├── services/                # 服务层
│   │   └── analysis_service.py  # 分析服务
│   └── ...                      # 其他核心模块
│
├── providers/                    # 云厂商实现
│   ├── aliyun/                  # 阿里云Provider
│   │   ├── provider.py          # 同步Provider
│   │   └── async_provider.py    # 异步Provider
│   └── tencent/                 # 腾讯云Provider
│       └── provider.py
│
├── resource_modules/             # 资源分析模块
│   ├── ecs_analyzer.py          # ECS分析器
│   ├── rds_analyzer.py          # RDS分析器
│   ├── redis_analyzer.py        # Redis分析器
│   ├── vpc_analyzer.py          # VPC分析器
│   ├── eip_analyzer.py          # EIP分析器
│   ├── disk_analyzer.py         # 磁盘分析器
│   ├── cost_analyzer.py         # 成本分析器
│   └── ...                      # 其他资源分析器
│
├── web/                          # Web应用
│   ├── backend/                 # 后端API服务
│   │   ├── main.py              # FastAPI应用入口
│   │   ├── api.py               # 主API路由
│   │   ├── api_alerts.py        # 告警API
│   │   ├── api_cost_allocation.py  # 成本分配API
│   │   ├── api_ai_optimizer.py # AI优化API
│   │   ├── error_handler.py    # 错误处理
│   │   └── i18n.py             # 国际化
│   └── frontend/                # 前端应用
│       ├── app/                 # Next.js App Router
│       │   ├── _pages/          # 页面组件
│       │   ├── a/[account]/    # 账号路由
│       │   └── ...
│       ├── components/          # React组件
│       ├── lib/                # 工具库
│       └── ...
│
├── models/                       # 数据模型
│   └── resource.py             # 统一资源模型
│
├── utils/                        # 工具模块
│   ├── logger.py               # 日志工具
│   ├── error_handler.py        # 错误处理
│   ├── retry_helper.py         # 重试助手
│   ├── concurrent_helper.py    # 并发助手
│   ├── credential_manager.py   # 凭证管理
│   └── cost_predictor.py       # 成本预测
│
├── scripts/                      # 工具脚本
│   ├── install_completion.sh   # 安装Shell自动补齐
│   ├── migrate_sqlite_to_mysql.py  # 数据迁移脚本
│   └── ...                     # 其他工具脚本
│
├── sql/                          # SQL脚本
│   ├── init_mysql_schema.sql    # MySQL表结构
│   └── verify_schema.py        #  schema验证脚本
│
├── tests/                        # 测试用例
│   ├── core/                    # 核心模块测试
│   ├── providers/               # Provider测试
│   ├── resource_modules/        # 资源模块测试
│   └── utils/                   # 工具模块测试
│
├── docs/                         # 文档
│   ├── WEB_QUICKSTART.md        # Web快速开始
│   ├── DISCOUNT_ANALYSIS_GUIDE.md  # 折扣分析指南
│   ├── BILL_AUTO_FETCH_GUIDE.md    # 账单获取指南
│   ├── PLUGIN_DEVELOPMENT.md    # 插件开发指南
│   └── shell_completion.md      # Shell自动补齐
│
├── examples/                     # 示例代码
│   ├── basic_usage.py           # 基础使用示例
│   └── advanced_usage.py       # 高级使用示例
│
├── completions/                  # Shell自动补齐
│   ├── cl-complete.bash         # Bash补齐脚本
│   └── cl-complete.zsh          # Zsh补齐脚本
│
├── .github/                      # GitHub配置
│   └── workflows/               # CI/CD工作流
│       └── ci.yml               # 持续集成配置
│
├── README.md                     # 项目主文档
├── QUICKSTART.md                # 快速开始指南
├── USER_GUIDE.md                # 用户手册
├── TECHNICAL_ARCHITECTURE.md    # 技术架构文档
├── PRODUCT_INTRODUCTION.md      # 产品介绍
├── CONTRIBUTING.md              # 贡献指南
├── CHANGELOG.md                 # 更新日志
├── requirements.txt             # Python依赖
├── pyproject.toml               # 项目配置
└── config.json.example          # 配置示例
```

## 🏗️ 核心模块说明

### CLI层 (`cli/`)
- **职责**: 命令行接口，用户交互
- **技术**: Click框架
- **主要文件**:
  - `main.py`: CLI入口，命令注册
  - `commands/*.py`: 各功能命令实现

### 核心业务层 (`core/`)
- **职责**: 业务逻辑实现
- **主要模块**:
  - **数据库层**: `database.py` - 数据库抽象，支持MySQL/SQLite
  - **存储管理**: `cache.py`, `bill_storage.py`, `dashboard_manager.py` 等
  - **分析引擎**: `idle_detector.py`, `cost_analyzer.py`, `discount_analyzer_advanced.py` 等
  - **安全合规**: `security_compliance.py`, `cis_compliance.py`
  - **服务层**: `services/analysis_service.py` - 分析服务封装

### 云厂商层 (`providers/`)
- **职责**: 云厂商API封装
- **设计模式**: 抽象工厂模式
- **实现**:
  - `BaseProvider`: 抽象基类，定义统一接口
  - `AliyunProvider`: 阿里云实现
  - `TencentProvider`: 腾讯云实现
  - 支持同步和异步两种模式

### 资源分析层 (`resource_modules/`)
- **职责**: 各类资源的专门分析逻辑
- **特点**: 插件化设计，易于扩展
- **分析器类型**:
  - ECS、RDS、Redis等资源分析器
  - 成本分析器
  - 网络分析器

### Web应用层 (`web/`)
- **后端** (`web/backend/`):
  - FastAPI框架
  - RESTful API设计
  - 支持中英文国际化
- **前端** (`web/frontend/`):
  - Next.js框架
  - React + TypeScript
  - 现代化UI设计（Finout风格）

## 🔧 技术栈

### 后端技术
- **语言**: Python 3.8+
- **框架**: FastAPI
- **数据库**: MySQL（主），SQLite（兼容）
- **ORM**: 自定义数据库抽象层
- **缓存**: MySQL/SQLite缓存表

### 前端技术
- **框架**: Next.js 14+
- **语言**: TypeScript
- **UI库**: 自定义组件库（Tailwind CSS）
- **状态管理**: React Context API
- **国际化**: 自定义i18n方案

### CLI技术
- **框架**: Click
- **交互**: prompt_toolkit, rich
- **配置**: Keyring（安全存储）

## 📊 数据存储

### MySQL数据库
项目使用MySQL作为主数据库，通过数据库抽象层支持SQLite兼容。

**主要表结构**:
- `resource_cache` - 资源查询缓存
- `bill_items` - 账单明细
- `dashboards` - 仪表盘配置
- `budgets` - 预算管理
- `budget_records` - 预算执行记录
- `alert_rules` - 告警规则
- `alerts` - 告警记录
- `virtual_tags` - 虚拟标签
- `cost_allocation` - 成本分配
- 等13+个表

### 配置文件
- **位置**: `~/.cloudlens/config.json`
- **内容**: 云账号配置（AccessKey等）
- **安全**: 支持Keyring加密存储

## 🔐 安全设计

1. **密钥管理**: 使用系统Keyring，不存储明文
2. **权限控制**: PermissionGuard检查API权限
3. **只读操作**: CLI默认只读，修复操作需明确确认
4. **审计日志**: 所有修复操作记录完整日志

## 🚀 扩展性

### 添加新云厂商
1. 在 `providers/` 下创建新目录
2. 实现 `BaseProvider` 接口
3. 注册到Provider工厂

### 添加新资源类型
1. 在 `resource_modules/` 创建分析器
2. 实现 `BaseResourceAnalyzer` 接口
3. 注册到分析器注册表

### 添加新分析功能
1. 在 `core/` 创建分析模块
2. 在 `cli/commands/analyze_cmd.py` 添加命令
3. 在 `web/backend/api.py` 添加API端点

## 📝 开发规范

### 代码风格
- Python: 遵循PEP 8
- TypeScript: 遵循ESLint规则
- 使用类型提示（Python）和TypeScript类型

### 测试
- 单元测试: `tests/` 目录
- 运行测试: `pytest tests/`

### 文档
- API文档: FastAPI自动生成（`/docs`）
- 代码注释: 使用docstring
- 用户文档: `docs/` 目录

## 🔄 版本管理

- **主版本**: 重大架构变更
- **次版本**: 新功能添加
- **修订版本**: Bug修复和小改进

当前版本: **v2.1.0**

---

**最后更新**: 2025-01-XX
