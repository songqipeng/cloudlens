        ...
        try:
            ...
            for disk in disk_list:
                disks.append({
                    ...
                    'Category': disk.get('Category', '未知'),  # 云盘类型：cloud、cloud_essd、cloud_ssd等
                    'Size': disk.get('Size', 0),  # 容量（GB）
                    'Device': disk.get('Device', ''),  # 挂载点，如/dev/xvda
                    'DiskChargeType': disk.get('DiskChargeType', ''),  # 付费类型：PrePaid/PostPaid
...
def main(tenant_name=None):
    ...
    for i, instance in enumerate(all_instances, 1):
        ...
        print(f"  [{i}/{len(all_instances)}] {instance_name} ({instance_id})...", end="", flush=True)
            ...
            if disks:
                for disk in disks:
                    all_disk_data.append({
                        ...
                        '性能级别': disk['PerformanceLevel'] if disk['PerformanceLevel'] else 'N/A',
                    })
    ...
    for range_name, (min_size, max_size) in size_ranges.items():
        ...
        if count > 0:
            total_size = sum(d['磁盘大小(GB)'] for d in all_disk_data if min_size <= d['磁盘大小(GB)'] <= max_size)
