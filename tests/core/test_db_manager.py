...
class TestDatabaseManager:
    ...
    def test_create_resource_table(self, db_manager):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='test_resource_instances'"
        )
    ...
    def test_create_monitoring_table(self, db_manager):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='test_resource_monitoring_data'"
        )
        assert cursor.fetchone() is not None
        conn.close()
