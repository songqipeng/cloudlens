...
class TestDiscountAnalyzer:
    ...
    def test_get_all_ecs_instances(self, mock_request_class, mock_client_class, analyzer):
        ...
        # Mock空页（结束分页）
        ecs_page2_response = json.dumps({"Instances": {"Instance": []}})

        mock_client = MagicMock()
        # 第一次调用返回第1页，第二次返回空页
        mock_client.do_action_with_exception.side_effect = [ecs_page1_response, ecs_page2_response]
    ...
    def test_get_renewal_prices_ecs_skip_postpaid(self, mock_client_class, mock_process, analyzer):
        ...
        # Mock并发处理返回跳过结果
        mock_process.return_value = [{"skip": True, "reason": "按量付费"}]
    ...
    @patch("resource_modules.discount_analyzer.AcsClient")
    @patch("resource_modules.discount_analyzer.process_concurrently")
    def test_analyze_ecs_discounts_empty(
        self, mock_process, mock_client_class, analyzer, tmp_path, capsys
    ):
        """测试分析ECS折扣 - 空实例"""
