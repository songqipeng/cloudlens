...
class TestRedisAnalyzer:
    ...
    def test_init_database(self, analyzer, temp_db, monkeypatch):
        ...
        # 检查redis_instances表
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='redis_instances'"
        )
        assert cursor.fetchone() is not None

        # 检查redis_monitoring_data表
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='redis_monitoring_data'"
        )
        assert cursor.fetchone() is not None
    ...
    def test_get_redis_instances_empty(self, mock_request_class, mock_client_class, analyzer):
        """测试获取Redis实例（空结果）"""
        mock_response = json.dumps({"Instances": {"KVStoreInstance": []}})
    ...
    def test_is_instance_idle_low_cpu(self):
        ...
        metrics = {"CpuUsage": 5.0, "MemoryUsage": 50.0, "ConnectionUsage": 50.0}  # < 10%
    ...
    def test_is_instance_idle_low_memory(self):
        ...
        metrics = {"CpuUsage": 50.0, "MemoryUsage": 15.0, "ConnectionUsage": 50.0}  # < 20%
    ...
    def test_is_instance_idle_low_connection(self):
        ...
        metrics = {"CpuUsage": 50.0, "MemoryUsage": 50.0, "ConnectionUsage": 10.0}  # < 20%
    ...
    def test_is_instance_idle_low_traffic(self):
        ...
        # 流量低应该被判定为闲置
        assert (
            "Intranet" in str(reasons)
            or "流量" in str(reasons)
            or "traffic" in str(reasons).lower()
        )
    ...
    def test_get_optimization_suggestion_low_usage(self):
        ...
        metrics = {"CpuUsage": 5.0, "MemoryUsage": 5.0, "ConnectionUsage": 5.0}
    ...
    def test_get_optimization_suggestion_normal(self):
        ...
        metrics = {"CpuUsage": 50.0, "MemoryUsage": 50.0, "ConnectionUsage": 50.0}
    ...
    @patch("resource_modules.redis_analyzer.AcsClient")
    @patch("resource_modules.redis_analyzer.CommonRequest")
    def test_get_redis_instances_with_different_types(
        self, mock_request_class, mock_client_class, analyzer
    ):
        """测试获取不同类型的Redis实例"""
        mock_response = json.dumps(
            {
        ...
        arch_types = [
            inst.get("ArchitectureType") for inst in instances if "ArchitectureType" in inst
        ]
